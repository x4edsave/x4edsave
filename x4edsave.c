...
// version testing 	9.0 Beta 606138
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <windows.h>
...
int main(int argc, char *argv[]) {
	...
	if (argc < 2) {
		printf("Using: %s <save_00X.xml> \"ALL\" (or selectively) \"statistics playerup playercargoup nologs xenoncargoup xenonup nomods nodef nocargo\" \n", argv[0]);
		return 1;
	}
	...
	WORD STATISTICS = 0;
	WORD PLAYER_POWERUP = 0;
	WORD PLAYER_CARGOUP = 0;
	WORD XENON_CARGOUP = 0;
	WORD XENON_POWERUP = 0; // repair hull; insert modifications
	WORD REMOVE_ECONOMY_LOGS = 0; // remove economylog
	WORD REMOVE_MODIFICATIONS_OTHER_SHIPS = 0; // remove modifications all fractions exclude player & xenon
	WORD REMOVE_DEFENCE_IN_STATIONS = 0; // remove shields, turrets, drones
	WORD REMOVE_CARGO_IN_STATIONS = 0; // remove cargo in stations & ships
	WORD LOG_SHIPS_PLAYER = 0; // print sting where player ships
	WORD EXPORT_SHIPS_PLAYER = 0; // print all player ships save data to tmp file
						...
						if (xenon_add_cargo_in) {
							...
							if (!xenon_shipyard) {
								...
								if (beginson(line, "<ware ware=\"energycells")) {
									...
									fprintf(output_file, "<ware ware=\"energycells\" amount=\"50000\"/>\n");
								}
								if (beginson(line, "<ware ware=\"ore")) {
									
									fprintf(output_file, "<ware ware=\"ore\" amount=\"50000\"/>\n");
								}
								if (beginson(line, "<ware ware=\"silicon")) {
									
									fprintf(output_file, "<ware ware=\"silicon\" amount=\"50000\"/>\n");
								}
