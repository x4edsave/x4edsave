...
#define XENON_POWERUP
//#define REMOVE_ECONOMY_LOGS
#define REMOVE_MODIFICATIONS_OTHER_SHIPS
#define REMOVE_DEFENCE_IN_STATIONS
//#define REMOVE_CARGO_IN_STATIONS
//#define LOG_SHIPS_PLAYER

//#define EXPORT_PLAYER_SHIPS
...
#ifdef LOG_SHIPS_PLAYER // ship owner player -> text log
						if ( !skip_ship ) {
...
#ifdef REMOVE_DEFENCE_IN_STATIONS
				remove_defence_in_station = 1;
#endif
#ifdef REMOVE_CARGO_IN_STATIONS
...
#ifdef REMOVE_MODIFICATIONS_OTHER_SHIPS
				if ( strstr(line, "/>") != NULL ) {
...
/*		
#ifdef REMOVE_MASSTRAFFIC
			if ( beginson(line, "<masstraffic>" )) {
				
				remove_masstraffic = 1;
				skip_lines = 1;
								
			}	
			
			if ( remove_masstraffic ) {
				
				if ( beginson(line, "</masstraffic>" )) {
				
					fprintf(output_file, "<masstraffic>\n");
					remove_masstraffic = 0;
					skip_lines = 0;
					//skip_one_line = 1;
...
*/		
		...
		///////////////
		///////////////
#ifdef EXPORT_PLAYER_SHIPS		
		if (this_player_ship && in_ship_connection && in_universe) {
			skip_lines = 0;
		} else {
			skip_lines = 1;
		}
